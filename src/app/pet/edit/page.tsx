...
export default function EditPetPage() {
  ...
  useEffect(() => {
    if (petId) {
      ...
      if (storedPets) {
        ...
        if (pet) {
          // 날짜 포맷 변환 (YYYY년 MM월 DD일 -> YYYY-MM-DD)
          ...
          // 몸무게에서 'kg' 제거
          const weightValue = pet.weight.replace('kg', '');
          ...
          setFormData({
            ...
            birthDate: convertDateFormat(pet.birthDate),
            weight: weightValue,
          });
  ...
  const handleSubmit = () => {
    ...
    const pets = JSON.parse(storedPets);
    const petIndex = pets.findIndex((p: any) => p.id === parseInt(petId));
    ...
    pets[petIndex] = {
      ...pets[petIndex],
      name: formData.name,
      image: formData.image || pets[petIndex].image,
  ...
  return (
    <div className="min-h-screen w-full max-w-mobile mx-auto bg-white flex flex-col relative">
      {/* 헤더 */}
      <Header title="반려동물 정보 수정" showBackButton={true} showIcons={true} />
